...
- CI/CD (Fase 8 concluída, validada em 24/08/2026): GitHub Actions com 4 jobs — `check` (qualidade + unitários), `integration` (Testcontainers), `smoke` (e2e docker-compose até saga terminal) e `build-images` (10 imagens → GHCR, **multi-arch** amd64+arm64). Deploy em cloud via Helm/Terraform planejado (Fase 10: EKS, MSK/RDS, ArgoCD).
